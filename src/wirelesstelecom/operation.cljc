(ns wirelesstelecom.operation
  "OperationActor -- one wireless-carrier operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Network Operations Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Mobile Network Governor
  ...
  One graph run = one wireless-carrier operation (intake -> advise ->
  ...
  decision to a human operator (the mobile network operator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/provision-msisdn`/`:actuation/suspend-
  service` ALWAYS reach this node when the governor is clean -- see
  `wirelesstelecom.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [wirelesstelecom.opsadvisor :as opsadvisor]
...
(defn build
  ...
  `wirelesstelecom.store/Store`).
  opts:
    :advisor      -- a `wirelesstelecom.opsadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (opsadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Network Operations Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (opsadvisor/-advise advisor store request)]
            {:proposal p :audit [(opsadvisor/trace request p)]})))

      ;; Mobile Network Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
